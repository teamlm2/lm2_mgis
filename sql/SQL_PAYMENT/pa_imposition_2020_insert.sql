...
where im.imposition_year = 2019 and substring(bayan.au2, 1, 3) = '085'
)xxx where rank = 1)
...
update data_estimate.pa_imposition set remainning_amount = nn.remaining_amount::numeric
from new_numbers nn
where data_estimate.pa_imposition.id = nn.id

-----------urd onii iluu dutuug tootsoh nogduulalt update hiih

update data_estimate.pa_imposition  set total_amount = year_amount - remainning_amount where remainning_amount != 0 and substring(au2, 1, 3) = '085' and imposition_year = 2019;
...
select fee.id, im.id, 2019, abs(bayan.remaining_amount::numeric), bayan.paid_amount+bayan.remaining_amount::numeric, bayan.paid_amount, im.total_amount::numeric-bayan.paid_amount::numeric, bayan.au2, bayan.parcel_id, c.contract_id, person.person_id, fee.fee_calculated, im.year_amount, im.total_amount, 1 from data_estimate.pa_imposition im
join data_soums_union.ct_contract c on im.contract_id = c.contract_id
join data_soums_union.ct_fee fee on fee.id = im.fee_id
join base.bs_person person on fee.person = person.person_id
join data_estimate.uvs_paid_2019 bayan on im.parcel_id = bayan.parcel_id and upper(c.contract_no) = upper(bayan.contract_no) and upper(person.person_register) = upper(bayan.person_register)
