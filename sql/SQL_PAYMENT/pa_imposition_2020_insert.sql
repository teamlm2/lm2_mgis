...
-------------shalgah
select * from data_estimate.paid_bayannuur_2019
...
join data_estimate.paid_bayannuur_2019 bayan on im.parcel_id = bayan.parcel_id and upper(c.contract_no) = upper(bayan.contract_no) and upper(person.person_register) = upper(bayan.person_register)
...
select fee.id, im.id, 2019, abs(bayan.remaining_amount), bayan.paid_amount+bayan.remaining_amount, bayan.paid_amount, im.total_amount-bayan.paid_amount, '06307', bayan.parcel_id, c.contract_id, person.person_id, fee.fee_calculated, im.year_amount, im.total_amount, 1 from data_estimate.pa_imposition im
join data_soums_union.ct_contract c on im.contract_id = c.contract_id
join data_soums_union.ct_fee fee on fee.id = im.fee_id
join base.bs_person person on fee.person = person.person_id
join data_estimate.paid_bayannuur_2019 bayan on im.parcel_id = bayan.parcel_id and upper(c.contract_no) = upper(bayan.contract_no) and upper(person.person_register) = upper(bayan.person_register)
