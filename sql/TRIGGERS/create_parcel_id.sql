...
BEGIN
  ...
  UPDATE settings.set_role_user SET pa_till = 'infinity' where user_name = current_user;
	...
	SELECT code FROM admin_units.au_level2 INTO v_admin_unit_l2_code WHERE ST_COVERS(geometry, NEW.geometry);
        ...
        execute 'SELECT max(substring(parcel_id from 6 for 5)::int) FROM data_soums_union.ca_union_parcel WHERE parcel_id LIKE $1' into v_counter using v_parcel_prefix || '%';
        ...
        if v_counter < v_ub_counter then
			...
			IF (NEW.parcel_id IS NULL) THEN
				...
				IF (v_cblock_code || lpad(v_counter::text, 5, '0')) IS NULL THEN
					execute 'SELECT max(substring(parcel_id from 11 for 7)::int) FROM data_soums_union.ca_union_parcel WHERE au2 = '''||v_admin_unit_l2_code||''' and parcel_id LIKE $1' into v_error_counter using 'ER%';
