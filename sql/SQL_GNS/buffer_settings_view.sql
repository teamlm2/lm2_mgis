...
(st_buffer(geometry, (b.buffer_value * (select degrees_value from data_landuse.set_degrees_to_meters where id = 5)/(select meters_value from data_landuse.set_degrees_to_meters where id = 5))))::geometry(Polygon, 4326) as geometry, 
p.au2, p.valid_from, p.valid_till 
from data_landuse.ca_landuse_type_tbl p, data_landuse.set_buffer_au2_landuse b
where p.landuse = b.landuse and p.au2 = b.au2 and is_active = true --and parcel_id = 7325379 
union all
select p.parcel_id, p.landuse, 
(st_buffer(geometry, (b.buffer_value * (select degrees_value from data_landuse.set_degrees_to_meters where id = 5)/(select meters_value from data_landuse.set_degrees_to_meters where id = 5))))::geometry(Polygon, 4326) as geometry, 
p.au2, p.valid_from, p.valid_till
from data_landuse.set_buffer_parcel b, data_soums_union.ca_parcel_tbl p
where p.parcel_id = b.parcel_id and b.parcel_type = 1
union all
select p.parcel_id, p.landuse, 
(st_buffer(geometry, (b.buffer_value * (select degrees_value from data_landuse.set_degrees_to_meters where id = 5)/(select meters_value from data_landuse.set_degrees_to_meters where id = 5))))::geometry(Polygon, 4326) as geometry, 
p.au2, p.valid_from, p.valid_till
from data_landuse.set_buffer_parcel b, data_soums_union.ca_spa_parcel_tbl p
where p.parcel_id = b.parcel_id and b.parcel_type = 6
union all
select p.parcel_id, p.landuse, 
(st_buffer(geometry, (b.buffer_value * (select degrees_value from data_landuse.set_degrees_to_meters where id = 5)/(select meters_value from data_landuse.set_degrees_to_meters where id = 5))))::geometry(Polygon, 4326) as geometry, 
p.au2, p.valid_from, p.valid_till
from data_landuse.set_buffer_parcel b, data_soums_union.ca_state_parcel_tbl p
where p.parcel_id = b.parcel_id and b.parcel_type = 5
)p
