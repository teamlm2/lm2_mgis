...
drop table if exists set_overlaps_parcels cascade;
create table set_overlaps_parcels
(
  in_landuse int references codelists.cl_landuse_type on update cascade on delete restrict,
  ch_landuse int references codelists.cl_landuse_type on update cascade on delete restrict,
  is_input_landuse boolean not null,
...
grant select, insert, update, delete on set_overlaps_parcels to land_office_administration;
grant select, insert, update, delete on set_overlaps_parcels to cadastre_update, contracting_update;
grant select on set_overlaps_parcels to cadastre_view, contracting_view, reporting;

insert into set_overlaps_parcels(in_landuse, ch_landuse, is_input_landuse) values (2205, 3411, false);
insert into set_overlaps_parcels(in_landuse, ch_landuse, is_input_landuse) values (2204, 3411, false);
insert into set_overlaps_parcels(in_landuse, ch_landuse, is_input_landuse) values (2206, 3411, false);
