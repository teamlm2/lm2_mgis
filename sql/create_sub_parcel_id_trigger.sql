...
BEGIN
 ...
 IF (NEW.geometry IS NOT NULL) THEN
	...
	SELECT parcel_id FROM data_soums_union.ca_parcel_tbl INTO parcel_no WHERE ST_COVERS(geometry, NEW.geometry);
	...
	ELSE		
		execute 'SELECT max(substring(sub_parcel_id from 11 for 2)::int) FROM data_soums_union.ca_sub_parcel_tbl WHERE sub_parcel_id LIKE $1' into v_counter using parcel_no || '%';
		IF v_counter IS NULL THEN
			v_counter := 0;
		END IF;
		v_counter := v_counter + 1;	
		NEW.sub_parcel_id := parcel_no || lpad(v_counter::text, 2, '0');
