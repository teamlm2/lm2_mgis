...
select '04410-'||'1-'||LPAD(row_number() over()::text, 5, '0'), 2, au1_name ||', '|| au2_name ||', '|| au3_name, true, 1, ((ST_DUMP(geom)).geom)::geometry(Polygon,4326), 2 from data_monitoring.aa_main_par

----sub_parcel

insert into data_monitoring.mt_monitor_parcel(code, landuse_real, address_neighbourhood, is_active, monitor_parcel_type_id, geometry, monitor_type_id)

select '04410-'||'2-'||LPAD(row_number() over()::text, 5, '0'), lcode2, au1_name ||', '|| au2_name ||', '|| au3_name, true, 2, ((ST_DUMP(geom)).geom)::geometry(Polygon,4326), 2 from data_monitoring.aa_sub_par
