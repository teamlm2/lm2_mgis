...
select gr.group_no, gr.group_name, con.contract_no, con.contract_no, pg.code, au1.code||':'||au1.name, au2.code||':'||au2.name from data_soums_union.ct_person_group gr
join data_soums_union.ct_group_member gm on gr.group_no = gr.group_no
join data_soums_union.ct_application_pug app_gr on gr.group_no = app_gr.group_no
join data_soums_union.ct_app_group_boundary app_pg on app_gr.application = app_pg.application
join data_soums_union.ca_pug_boundary pg on pg.code = app_pg.boundary_code
join data_soums_union.ct_contract_application_role con_app on app_pg.application = con_app.application
join data_soums_union.ct_contract con on con_app.contract = con.contract_id
join data_soums_union.ct_application_document app_doc on app_pg.application = app_doc.application_id
join data_soums_union.ct_application_pug_parcel app_p on app_pg.application = app_p.application
join data_soums_union.ct_application_parcel_pasture app_parcel on app_pg.application = app_parcel.application
join data_soums_union.ca_pasture_parcel_tbl parcel on app_parcel.parcel = parcel.parcel_id
join admin_units.au_level2 au2 on gr.au2 = au2.code
join admin_units.au_level1 au1 on au2.au1_code = au1.code

where parcel.group_type = 1
